    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8">
      <div className="p-6 rounded-3xl bg-gradient-to-r from-slate-900 via-purple-950 to-indigo-950 text-white shadow-xl flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          {onOpenPstpShield && (
            <button onClick={onOpenPstpShield} className="px-4 py-2.5 rounded-2xl bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-black text-xs shadow-lg flex items-center gap-2 hover:opacity-95 transition-opacity">
      ...
      <div className="flex border-b border-slate-200 dark:border-slate-800 gap-2 overflow-x-auto">
        <button onClick={() => setActiveTab('orders')} className={`pb-3 px-4 text-xs font-bold border-b-2 flex items-center gap-2 transition-colors shrink-0 ${activeTab === 'orders' ? 'border-purple-600 text-purple-600 dark:text-purple-400' : 'border-transparent text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>
          <Package className="w-4 h-4 text-indigo-400" /><span>Orders & Fulfillment Engine</span>
        </button>
        <button onClick={() => setActiveTab('trust')} className={`pb-3 px-4 text-xs font-bold border-b-2 flex items-center gap-2 transition-colors shrink-0 ${activeTab === 'trust' ? 'border-purple-600 text-purple-600 dark:text-purple-400' : 'border-transparent text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>
          <ShieldCheck className="w-4 h-4 text-purple-500" /><span>Trust & Reputation Center</span>
        </button>
        <button onClick={() => setActiveTab('i18n')} className={`pb-3 px-4 text-xs font-bold border-b-2 flex items-center gap-2 transition-colors shrink-0 ${activeTab === 'i18n' ? 'border-purple-600 text-purple-600 dark:text-purple-400' : 'border-transparent text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>
          <Globe className="w-4 h-4 text-amber-400" /><span>Language & Localization (i18n)</span>
        </button>
        <button onClick={() => setActiveTab('overview')} className={`pb-3 px-4 text-xs font-bold border-b-2 flex items-center gap-2 transition-colors shrink-0 ${activeTab === 'overview' ? 'border-purple-600 text-purple-600 dark:text-purple-400' : 'border-transparent text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>
          <Activity className="w-4 h-4" /><span>System Overview</span>
        </button>
        <button onClick={() => setActiveTab('utility')} className={`pb-3 px-4 text-xs font-bold border-b-2 flex items-center gap-2 transition-colors shrink-0 ${activeTab === 'utility' ? 'border-purple-600 text-purple-600 dark:text-purple-400' : 'border-transparent text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>
        ...
        <button onClick={() => setActiveTab('disputes')} className={`pb-3 px-4 text-xs font-bold border-b-2 flex items-center gap-2 transition-colors shrink-0 ${activeTab === 'disputes' ? 'border-purple-600 text-purple-600 dark:text-purple-400' : 'border-transparent text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>
          <AlertTriangle className="w-4 h-4 text-amber-500" /><span>Order Dispute Center</span>
        </button>
      </div>
      ...
      {activeTab === 'utility' && utilityConfig && onUpdateUtilityConfig && (
        <UtilityAdminPanel config={utilityConfig} onUpdateConfig={onUpdateUtilityConfig} rateLogs={utilityRateLogs} />
      )}
          ...
          <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">Merchant applications are reviewed only in the server-authoritative Merchant Compliance Queue.</p>
              </div>
            ...
            {vendorApplications.length === 0 ? (
              ...
              <div className="space-y-3">
                {vendorApplications.map((app) => (
                  <div key={app.id} className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-3">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="flex items-center gap-3">
                        <img src={app.logoUrl || app.storeLogo || 'https://images.unsplash.com/photo-1534452203293-494d7ddbf7e0?auto=format&fit=crop&w=200&q=80'} alt={app.storeName} className="w-12 h-12 rounded-xl object-cover border border-slate-700" />
                        <div>
                          <div className="flex items-center gap-2">
                            <h4 className="font-bold text-sm text-white">{app.storeName}</h4>
                            <span className="text-[10px] font-extrabold px-2 py-0.5 rounded bg-purple-500/20 text-purple-300">@{app.pioneerUsername}</span>
                            <span className={`text-[10px] font-extrabold px-2 py-0.5 rounded ${app.status === 'APPROVED' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : app.status === 'REJECTED' ? 'bg-rose-500/20 text-rose-400 border border-rose-500/30' : 'bg-amber-500/20 text-amber-400 border border-amber-500/30'}`}>{app.status}</span>
                          </div>
                      ...
                      <div className="px-3.5 py-1.5 rounded-xl bg-indigo-600/15 border border-indigo-500/25 text-indigo-300 text-xs font-bold">
                        Compliance Queue Only
                      </div>
                    </div>
        ...
        <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-4">
          ...
          <div className="p-4 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 flex items-center justify-between text-xs">
            ...
            <div className="flex gap-2">
              <button onClick={() => onResolveDispute('ORD-PI-892341', 'release')} className="px-3 py-1 bg-emerald-600 text-white font-bold rounded-lg">Settle Order to Seller</button>
              <button onClick={() => onResolveDispute('ORD-PI-892341', 'refund')} className="px-3 py-1 bg-rose-600 text-white font-bold rounded-lg">Refund Buyer</button>
            </div>
          </div>
        </div>
      )}

      {activeTab === 'finance' && <FinanceAnalyticsView products={INITIAL_PRODUCTS} orders={safeOrders} onOpenPstpShield={onOpenPstpShield} />}
      ...
      {activeTab === 'overview' && (
        <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
            ...
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-700 space-y-1">
