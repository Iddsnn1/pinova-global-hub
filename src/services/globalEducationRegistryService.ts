...
function mapNcesInstitution(row: NcesRow): InstitutionProfile | null {
  ...
  const name = String(row.INSTNM || '').trim();
  const state = String(row.STABBR || '').toUpperCase();
  if (!unitId || !name || !US_STATE_CODES.has(state)) return null;
  const now = new Date().toISOString();
  const website = String(row.WEBADDR || '').trim();
  const institutionType: InstitutionProfile['institutionType'] = row.ICLEVEL === 1 ? 'university' : row.ICLEVEL === 2 ? 'college' : 'vocational_institute';
  return {
    id: `inst-us-ipeds-${unitId}`,
    name,
    legalName: name,
    tradingName: name,
    institutionCode: `IPEDS-${unitId}`,
    institutionType,
    ...
    logoUrl: '',
    isPublic: Number(row.CONTROL) === 1,
    curriculum: ['U.S. postsecondary education — NCES IPEDS directory record'],
    instructionLanguage: ['English'],
    isBoarding: false,
    hasDayOption: true,
    accreditation: {
      authority: 'U.S. Department of Education — NCES IPEDS',
      registrationNumber: String(unitId),
      accreditationStatus: 'under_review',
      verifiedAt: now,
      officialNotes: 'Institution identity is verified against the NCES IPEDS directory. Academic hierarchy and accreditation are not inferred.'
    },
...
async function loadNcesUsInstitutions(): Promise<InstitutionProfile[]> {
  ...
  for (let offset = 0; offset < 10000; offset += 1000) {
    const params = new URLSearchParams({
      where: '1=1',
      outFields: 'UnitID,INSTNM,ADDR,CITY,STABBR,WEBADDR,CONTROL,ICLEVEL',
      returnGeometry: 'false',
      resultRecordCount: '1000',
      resultOffset: String(offset),
      orderByFields: 'INSTNM ASC',
      f: 'json'
    });
  ...
  ncesCache = { expiresAt: Date.now() + 15 * 60 * 1000, institutions };
  return institutions;
}
...
educationService.getInstitutions = async (filter) => {
  ...
  try {
    ...
    const remote = await loadNcesUsInstitutions();
    ...
    console.warn('[Global Education Registry] NCES IPEDS unavailable; retaining existing U.S. directory:', error);
