import { authenticateRequest } from '../../src/server/services/DurableVendorAuth';
...
export default async function handler(req: any, res: any) {
  ...
  const access = {
    applicationFound: Boolean(application), applicationStatus: application?.status || null,
    verificationStatus: application?.verificationStatus || 'Unverified', sellerStatus: application?.sellerStatus || 'Inactive',
    canSell: approved, canReceivePstpOrders: approved && application?.pstpAgreementAccepted === true
  };
  return res.json({ success: true, authorized: approved, status: application?.status || 'UNREGISTERED', sellerLifecycle: approved ? 'ACTIVE' : 'INACTIVE', verified: approved, pstpAuthorized: access.canReceivePstpOrders, storeName: application?.storeName || null, applicationId: application?.id || null, access });
}
