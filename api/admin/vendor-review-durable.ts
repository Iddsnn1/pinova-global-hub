import { authenticateVendorAdminRequest as authenticateAdminRequest, getDurableVendorApplication, listDurableVendorApplications, saveDurableVendorApplication, durableVendorStorageEnabled, pstpAuditRepo } from '../../dist/server.cjs';
...
export default async function handler(req: any, res: any) {
  ...
  pstpAuditRepo.appendLog({ orderId: id, actor: user.username, actorRole: 'admin', action: `VENDOR_APPLICATION_${status}`, details: `Application ${id} status updated to ${status}.`, ipAddress: req.ip || '127.0.0.1', deviceInfo: req.headers?.['user-agent'] || 'Admin Console' });
